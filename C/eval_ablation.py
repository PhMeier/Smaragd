...
def align_vars_baseline(amr, firstamr):
    ...
    :return alig:
    """
    matches_amr1 = re.findall("\w+\d \/ [\w+\d-]*", firstamr)
    matches_amr2 = re.findall("\w+\d \/ [\w+\d-]*", amr)
    alig = ""
...
def _overlap_pair(amr1str, amr2str):
    """
    ...
    # amr2 = penman....(amr2)
    try:
        amr1 = penman.decode(amr1str)
        amr2 = penman.decode(amr2str)
        amr1 = amr1.triples
        amr2 = amr2.triples
...
def compute_overlap_pair(alignment, firstamr, secondamr, make_baseline, smart_baseline):
    ...
    print("Alig: ", alignment)
    overlap, union, f1, p, r = _overlap_pair(firstamr, secondamr)  # norm, overlap
    try:
        return overlap / union, f1, p, r, len(alignment), alignment  # overlap / norm
    except ZeroDivisionError:
        return 0, 0, 0, 0, len(alignment), alignment


def compute_overlap_sample_constrained(alignments, firstamrs, secondamrs, make_baseline, smart_baseline, lower_bound,
                                       upper_bound):
    """
    ...
    for i, amr in enumerate(firstamrs):
        if len(amr) + len(secondamrs[i]) <= 99999:
            ...
            if num_amr1 > lower_bound and num_amr2 > lower_bound and num_amr1 < upper_bound and num_amr2 < upper_bound:
                score, f1, p, r, l = compute_overlap_pair(alignments[i], amr, secondamrs[i], make_baseline,
                                                          smart_baseline)  # _overlap_pair(alignments[i], amr) #compute_overlap_pair(alignments[i], amr, secondamrs[i])
                scores.append(score)
                f1_scores.append(f1)
                p_scores.append(p)
                r_scores.append(r)
...
def compute_overlap_sample(alignments, firstamrs, secondamrs, make_baseline, smart_baseline):
    ...
    for i, amr in enumerate(firstamrs):
        if len(amr) + len(secondamrs[i]) <= 99999:
            score, f1, p, r, align_len, alignment = compute_overlap_pair(alignments[i], amr, secondamrs[i],
                                                                         make_baseline, smart_baseline)
            scores.append(score)
            f1_scores.append(f1)
            p_scores.append(p)
            r_scores.append(r)
            indices.append(i)
            align_lens.append(align_len)
    ...
    return scores, f1_scores, p_scores, r_scores, align_lens, alignments_
...
def evaluation_procedure(gold_align, pred_align, amr1, amr2, study_bool, make_baseline, mutlisents_index, smart_baseline, lower_bound,
                         upper_bound):
    ...
    if study_bool:
        ...
        gold, f1_scores_g, p_scores_g, r_scores_g, abc, _ = compute_overlap_sample(gold_align, amr1, amr2, False,
                                                                                   smart_baseline)
        ...
        # amr1 = amr1[:10]
        # amr2 = amr2[:10]
        # 16/02 Austausch mit Gold alignments
        if make_baseline:
            pred, f1_scores_p, p_scores_p, r_scores_p, align_lens, alignments_ = compute_overlap_sample(gold_align,
                                                                                                        amr1, amr2,
                                                                                                        make_baseline,
                                                                                                        smart_baseline)
        else:
            pred, f1_scores_p, p_scores_p, r_scores_p, align_lens, alignments_ = compute_overlap_sample(pred_align,
                                                                                                        amr1, amr2,
                                                                                                        make_baseline,
                                                                                                        smart_baseline)
    ...
    # print("Alignment lens: ", sum(align_lens)/len(align_lens))
    ...
    alignments_ = splitter(alignments_)
    p_al = splitter(pred_align)
    g_al = splitter(gold_align)
    print("Alignment Accuracy: ", len(set(alignments_).intersection(g_al)) / len(set(alignments_).union(g_al)))
    print("Alignment Accuracy Scores: ", len(set(pred).intersection(gold)) / len(set(pred).union(gold)))
...
def filter_for_multisents(test_source, gold_align, pred_align):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_source", help="Test source data", required=True, type=str)  # "c_test.src"
    ...
    parser.add_argument("--lower_bound",
                        help="Lower bound used for constrained evaluation. Required for constrained study. Default value is 35.",
                        required=False, default=35, type=int)
    parser.add_argument("--upper_bound",
                        help="Upper bound used for constrained evaluation. Required for constrained study. Default value is 130.",
    ...
    single = [i for i in single_sentences if i not in mutlisents_index]
    print(single)
    mutlisents_index = mutlisents_index
    test_source = filter_list(test_source, mutlisents_index)
    gold_align = filter_list(gold_align, mutlisents_index)
    pred_align = filter_list(pred_align, mutlisents_index)
    """

    path = "C:/Users/phMei/PycharmProjects/AMR_ablation/C"
    test_source = read_file_per_line(args.test_source)  # "c_test.src")
    gold_align = read_file_per_line(args.gold_align)  # "c_test.tgt")
    pred_align = read_file_per_line(args.pred_align)  # "00026000.hyps.test")
    ...
    if args.multisentence:
        ...
        test_source, gold_align, pred_align, mutlisents_index = filter_for_multisents(test_source, gold_align,
                                                                                      pred_align)
    else:
    ...
    for i in range(10):
        a, b, c, d, e, f = evaluation_procedure(gold_align, pred_align, amr1, amr2, study_bool, make_baseline, mutlisents_index,
                                                smart_baseline, lower_bound, upper_bound)
        f1_pred += a
        prec_pred += b
        rec_pred += c
        std_dev_pred += d
